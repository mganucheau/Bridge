...
// ─────────────────────────────────────────────────────────────────────────────
int GuitarEngine::rootMidiBase() const
{
...
// ─────────────────────────────────────────────────────────────────────────────
int GuitarEngine::degreeToMidiNote (int degree, int prevMidi) const
{
...
void GuitarEngine::generatePatternRange (int fromStep0, int toStep0, bool seamlessPerform, BridgeMLManager* ml)
{
    ...
    for (int step = fromStep0; step <= toStep0; ++step)
    {
        ...
        const GuitarHit& prev = previous[(size_t) step];
    ...
    if (! partialRegenWindow && ml != nullptr && ml->hasModel (BridgeMLManager::ModelType::MelodyModel))
    {
...
// ─────────────────────────────────────────────────────────────────────────────
void GuitarEngine::rebuildGridPreview()
{
    ...
    for (int s = 0; s < patternLen; ++s)
    {
        if (! pattern[(size_t) s].active)
            continue;
        GuitarHit& out = displayPattern[(size_t) s];
...
void GuitarEngine::morphPatternForDensityAndComplexity (int rangeFromStep0, int rangeToStep0)
{
    ...
    while (activeInRange > targetInRange)
    {
        ...
        if (bestS < 0)
            break;
        pattern[(size_t) bestS] = GuitarHit{};
        --activeInRange;
    }
    ...
    for (int s = r0; s <= r1; ++s)
    {
        ...
        const float roll = (const_cast<GuitarEngine*> (this)->rng() / float (std::mt19937::max()));
...
void GuitarEngine::adaptPatternToNewStyle (int newStyleIndex)
{
    newStyleIndex = juce::jlimit (0, GuitarPreset::NUM_STYLES - 1, newStyleIndex);
    style = newStyleIndex;
...
void GuitarEngine::evolvePatternRangeForJam (int fromStep0, int toStep0, BridgeMLManager* ml)
{
...
// ─────────────────────────────────────────────────────────────────────────────
void GuitarEngine::generateFill (int fromStep)
{
...
float GuitarEngine::sampleProb (float p) const
{
    float adjusted = std::pow (jlimit (0.0f, 1.0f, p), 1.0f / temperature);
    return (const_cast<GuitarEngine*>(this)->rng() / float (std::mt19937::max())) < adjusted;
}

uint8 GuitarEngine::sampleVelocity (int step, bool ghost, bool accent) const
{
    int vMin, vMax;
    if (ghost)
    {
    ...
    int vel = const_cast<GuitarEngine*>(this)->rng() % jmax (1, vMax - vMin + 1) + vMin;
    vel     = (int)(vel * velocityMul);
    return (uint8)jlimit (1, 127, vel);
}

int GuitarEngine::chooseDegreeForMorphAdd (int step, int preferredDegree, int prevMidi) const
{
    if (prevMidi < 0)
...
int GuitarEngine::chooseDegreeProbabilistic (int step, int preferredDegree) const
{
    ...
    if (sampleProb (deviateProbability))
    {
        ...
        int idx = const_cast<GuitarEngine*>(this)->rng() % 6;
        return alts[idx];
    }
...
float GuitarEngine::complexityMod (int step) const
{
...
void GuitarEngine::resolveApproachNotes()
{
...
int GuitarEngine::resolveApproachNote (int step) const
{
...
void GuitarEngine::applyHumanize (double samplesPerStep)
{
...
// ─────────────────────────────────────────────────────────────────────────────
int GuitarEngine::getSwingOffset (int step, double samplesPerStep) const
{
...
// ─────────────────────────────────────────────────────────────────────────────
int GuitarEngine::calcNoteDuration (const GuitarHit& hit, double samplesPerStep) const
{
    ...
    if (!hit.isGhost)
    {
        float stacScale = 1.0f - staccato * 0.70f;
        artDur *= stacScale;
    }

    artDur *= 1.0f + hold * 0.55f;

    // Articulation: palm-mute shortens notes; strum intensity opens them up.
    if (! hit.isGhost)
        artDur *= juce::jmap (palmMute, 0.f, 1.f, 1.0f, 0.45f)
                * juce::jmap (strumIntensity, 0.f, 1.f, 0.85f, 1.20f);
...
// shouldTriggerFill
// ─────────────────────────────────────────────────────────────────────────────
bool GuitarEngine::shouldTriggerFill()
{
    ++barCount;
    if (fillRate < 0.01f) return false;

    float roll = const_cast<GuitarEngine*>(this)->rng() / float (std::mt19937::max());
