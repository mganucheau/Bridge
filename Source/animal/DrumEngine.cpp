...
void DrumEngine::generatePattern (bool seamlessPerform)
{
    ...
    for (int step = 0; step < patternLen; ++step)
    {
        for (int drum = 0; drum < NUM_DRUMS; ++drum)
        {
            ...
            // Complexity adds off-beat variation
            float prob = base + complexityMod (step, drum);

            // Density scales everything uniformly
            prob *= density * 1.5f; // ×1.5 so density=0.7 ≈ full pattern
    ...
    if (seamlessPerform && patternLen > 0)
    {
        const int blendSteps = juce::jmin (4, patternLen);
        for (int s = 0; s < blendSteps; ++s)
        {
            const float a = (float)(s + 1) / 5.0f;
            for (int d = 0; d < NUM_DRUMS; ++d)
            {
                const auto& o = previous[(size_t) s][(size_t) d];
                auto&       n = pattern[(size_t) s][(size_t) d];
                const bool  any = o.active || n.active;
                const int   vel = (int)((float) o.velocity * (1.0f - a) + (float) n.velocity * a);
                n.active   = any;
                n.velocity = (uint8) juce::jlimit (1, 127, vel);
            }
        }
    }
