...
// generatePattern
// ─────────────────────────────────────────────────────────────────────────────
void GuitarEngine::generatePattern()
{
    if (locked) return;
    ...
    for (int step = 0; step < patternLen; ++step)
    {
        ...
        // Complexity adds probability on off-beat 16th positions
        float prob = base + complexityMod (step);

        // Density scales everything uniformly
        prob *= density * 1.4f;
        ...
        if (active)
        {
            ...
            bool  isGhost    = (deg != 6) && sampleProb (ghostTend);  // approach notes are never ghost

            bool  isAccent   = (step % 4 == 0) && !isGhost;
            uint8 vel        = sampleVelocity (step, isGhost, isAccent);

            // Resolve MIDI note (approach target resolved in post-pass below)
            int midi = degreeToMidiNote (deg, -1);

            pattern[(size_t)step].active    = true;
            pattern[(size_t)step].velocity  = vel;
...
// ─────────────────────────────────────────────────────────────────────────────
// rebuildGridPreview — pattern shown in UI; optional live fill-hold layer (Animal-style)
// ─────────────────────────────────────────────────────────────────────────────
void GuitarEngine::rebuildGridPreview()
{
    ...
    for (int s = 0; s < patternLen; ++s)
    {
        ...
        GuitarHit& h = displayPattern[(size_t) s];
...
void GuitarEngine::applyHumanize (double samplesPerStep)
{
    ...
    for (int step = 0; step < patternLen; ++step)
    {
        ...
        // Random jitter
        int jitter = (int)(const_cast<GuitarEngine*>(this)->rng() % (maxShift * 2 + 1)) - maxShift;
