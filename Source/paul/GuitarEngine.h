...
using GuitarPattern = std::array<GuitarHit, PaulPreset::NUM_STEPS>;
...
public:
    ...
    // ── Parameters ─────────────────────────────────────────────────────────
    void setStyle       (int  s) { style       = jlimit (0, PaulPreset::NUM_STYLES - 1, s); }
    void setScale       (int  s) { scale       = jlimit (0, PaulPreset::NUM_SCALES - 1, s); }
    ...
    void setPatternLen  (int   l){ patternLen  = jlimit (1, PaulPreset::NUM_STEPS, l); }
...
private:
    ...
    int   patternLen  = PaulPreset::NUM_STEPS;
    bool  locked      = false;
    uint32 seed       = 42;
    int   barCount    = 0;
    int   phraseBars  = 4;
