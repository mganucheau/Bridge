...
void GuitarGridComponent::paint (juce::Graphics& g)
{
    ...
    const int tdIdxG = bridge::phrase::readTimeDivisionChoiceIndex (proc.apvtsMain);
    const int accentPeriodG = bridge::phrase::accentColumnPeriodInSixteenthsFromTimeDivisionIndex (tdIdxG);
    for (int step = 0; step < nSteps; ++step) {
        float cx = (float) step * cellW;
        const int stepInBar = step % bridge::phrase::kStepsPerBar;
        const bool isAccentCol = (stepInBar % accentPeriodG) == 0;
      ...
      instrumentStrip (InstrumentControlBar::makeGuitarConfig (p))
{
    ...
    {
        const int n = proc.guitarEngine.getPatternLen();
