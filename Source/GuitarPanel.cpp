...
{
    using namespace GuitarM3; // fallback
...
GuitarPanel::GuitarPanel (BridgeProcessor& p)
    : proc (p),
      bottomHalf (p.apvtsGuitar, laf, bridge::colors::accentGuitar,
        [this] { proc.triggerGuitarGenerate(); },
        [this] (bool active) { proc.guitarEngine.setFillHoldActive (active); })
{
    setLookAndFeel (&laf);

    proc.apvtsGuitar.addParameterListener ("loopStart", this);
    proc.apvtsGuitar.addParameterListener ("loopEnd", this);
    proc.apvtsGuitar.addParameterListener ("loopOn", this);
    ...
    styleAttach = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>
                      (proc.apvtsGuitar, "style", styleBox);
    auto setupCombo = [&](juce::Label& lbl, juce::ComboBox& box, const juce::String& title, const juce::String& paramId) {
        ...
        if (proc.apvtsGuitar.getParameter(paramId)) {
            if (title == "ROOT") rootAttach = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment> (proc.apvtsGuitar, paramId, box);
            else if (title == "SCALE") scaleAttach = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment> (proc.apvtsGuitar, paramId, box);
            else if (title == "OCT") octaveAttach = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment> (proc.apvtsGuitar, paramId, box);
        }
    };
...
namespace
{
    void layoutGuitarDropdownRow (juce::Rectangle<int> row,
...
void GuitarPanel::resized()
{
    ...
    layoutGuitarDropdownRow (dropdownRow, styleLabel, styleBox, rootLabel, rootBox, scaleLabel, scaleBox, octaveLabel, octaveBox);
