...
using PianoPattern = std::array<PianoHit, SteviePreset::NUM_STEPS>;
...
public:
    ...
    // ── Pattern access ─────────────────────────────────────────────────────
    const PianoHit&     getStep    (int step) const { return pattern[(size_t)step]; }
    const PianoPattern& getPattern ()         const { return pattern; }
    const PianoPattern& getPatternForGrid()   const { return displayPattern; }
    ...
    // ── Parameters ─────────────────────────────────────────────────────────
    void setStyle       (int  s) { style       = jlimit (0, SteviePreset::NUM_STYLES - 1, s); }
    void setScale       (int  s) { scale       = jlimit (0, SteviePreset::NUM_SCALES - 1, s); }
    ...
    void setPatternLen  (int   l){ patternLen  = jlimit (1, SteviePreset::NUM_STEPS, l); }
    ...
    int  calcNoteDuration (const PianoHit& hit, double samplesPerStep) const;
...
private:
    ...
    int   rootNote    = 0;   // 0=C
    int   octave      = 4;   // piano mid register (~C4)
    ...
    int   patternLen  = SteviePreset::NUM_STEPS;
    bool  locked      = false;
    uint32 seed       = 42;
    int   barCount    = 0;
    int   phraseBars  = 4;
