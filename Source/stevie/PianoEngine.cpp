...
// generatePattern
// ─────────────────────────────────────────────────────────────────────────────
void PianoEngine::generatePattern()
{
    if (locked) return;
    ...
    for (int step = 0; step < patternLen; ++step)
    {
        ...
        pattern[(size_t)step] = PianoHit{};  // clear
...
void PianoEngine::rebuildGridPreview()
{
    ...
    for (int s = 0; s < patternLen; ++s)
    {
        ...
        PianoHit& h = displayPattern[(size_t) s];
...
void PianoEngine::applyHumanize (double samplesPerStep)
{
    ...
    for (int step = 0; step < patternLen; ++step)
    {
        ...
        // Random jitter
        int jitter = (int)(const_cast<PianoEngine*>(this)->rng() % (maxShift * 2 + 1)) - maxShift;
